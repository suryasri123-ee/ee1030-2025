...
\begin{enumerate}
\item  In a $n\times n $ identity matrix, the trace equals: \hfill\brak{GATE-MT2009}
...
\item  Gibbs free energies of a system in states $1$ and $2$ are denoted by $G_1$ and $G_2$ respectively. The system will go spontaneously from state $1$ to state $2$ , if and only if: \hfill\brak{GATE-MT2009}
\begin{multicols}{4}
\begin{enumerate}
\item $G1-G2>0$
\item $G1-G2<0$
...
\item  In an ideal HCP packing , the $\frac{c}{a}$ ratio is:\hfill\brak{GATE-MT2009}
...
\item  Intensive thermodynamic variables are \hfill\brak{GATE-MT2009}
\begin{enumerate}
...
\item In a sound casting , the last liquid to solidify is in the \hfill\brak{GATE-MT2009}
\begin{multicols}{4}
\begin{enumerate}
\item runner
\item riser
...
\item An annealed plain carbon steel, showing fully pearlitic microstructure , has a carbon content of\hfill\brak{GATE-MT2009}
\begin{multicols}{4}
\begin{enumerate}
\item $0.001wt\%$
\item $0.20wt\%$
...
\item Superalloys are \hfill\brak{GATE-MT2009}
\begin{multicols}{2}
\begin{enumerate}
...
\end{enumerate}
\end{multicols}
\item The function, $f(x)=ax^2+bx+c$ has a maximum only if \hfill\brak{GATE-MT2009}
\begin{multicols}{4}
\begin{enumerate}
        \item$a<o$
        \item $a>0$
...
\end{enumerate}
\end{multicols}
\item A furnace wall consists of four layers of different materials M1,M2,M3 and M4. If the layers are of equal thickness and the steady state temperature profile is, as shown below , then the material with the lowest thermal conductivity is\hfill\brak{GATE-MT2009}
\begin{figure}[h]
    \centering
...
R.Ni\\
S.Zn\\
two metals which provide cathodic protection to steel are\hfill\brak{GATE-MT2009}
\begin{multicols}{4}
\begin{enumerate}
    \item P,R
    \item R,S
...
\item The Miller indices of the plane PQRS, shown in the unit cell are\hfill\brak{GATE-MT2009}
\begin{figure}[h]
    \centering
...
\item  $\lim_{x \to 0} \frac{sinx}{x}$ is equal to\hfill\brak{GATE-MT2009}
...
\item  X-ray radiography is used to determine the \hfill\brak{GATE-MT2009}
\begin{multicols}{2}
\begin{enumerate}
...
\item Hardenability of steel does NOT depend on the\hfill\brak{GATE-MT2009}
\begin{multicols}{2}
\begin{enumerate}
\item alloy content
\item grain size
...
\item p-type semiconductor can be obtained by doping silicon with\hfill\brak{GATE-MT2009}
\begin{multicols}{4}
\begin{enumerate}
\item antimony
\item phosphorous
...
\item The figure below shows water over mercury manometer. if the density of water is denoted by$\rho_w$ and that of mercury by $\rho_M$ and 'g' denotes the acceleration due to gravity , the pressure difference $(P_A - P_B)$ will be equal to\hfill\brak{GATE-MT2009}
\begin{figure}[h]
    \centering
...
\end{enumerate}
\end{multicols}
\item Match the processes given in Group 1 with the coresponding typical defects given in group 2\hfill\brak{GATE-MT2009}\\
\begin{center}
\begin{tabular}{c c}
...
\end{enumerate}
\end{multicols}
\item From the list given below , two factors that promotes coring in cast alloys are                \hfill\brak{GATE-MT2009}\\
...
\end{enumerate}
\end{multicols}
\item Match the loading conditions in group 1 with the characteristics in group 2.\hfill\brak{GATE-MT2009}
\begin{center}
\begin{tabular}{c c}
Group 1     &Group2  \\
...
\end{enumerate}
\end{multicols}
\item  Match the extraction methods in group 1 with the metals in group 2 \hfill\brak{GATE-MT2009}
\begin{center}
\begin{tabular}{c c}
Group 1     &Group 2  \\
...
\item  The average molecular weight of high density polyethylene is found to be $56000$.The degree of polymerization is \hfill\brak{GATE-MT2009}
\begin{multicols}{4}
\begin{enumerate}
\item $200$
\item $1000$
...
\item  A unit dislocation with a Burgers vector $\vec{b_1}$ will dissociate into two partial dislocations with burgers vectors $\vec{b_2}$ and $\vec{b_3}$ , if and only if  \hfill\brak{GATE-MT2009}\\
...
\item What is the magnitude of the following integral using single step appLication of trapezoid rule?\hfill\brak{GATE-MT2009}
\begin{center}
...
\item During a sheet stamping operation, it is observed that sheet surface area triples . the true thickness strain is  \hfill\brak{GATE-MT2009}
\begin{multicols}{4}
\begin{enumerate}
\item $-1.1$
\item $-0.333$
...
\end{enumerate}
\end{multicols}
\item Match the practices in Group 1 with reactors in Group 2.\hfill\brak{GATE-MT2009}
\begin{center}
\begin{tabular}{c c}
Group 1     &  Group 2\\
...
\item  Stacking fault energy (SFE) plays an important role in determining the work hardening ability of a metal. In this context, the correct logical sequence is \hfill\brak{GATE-MT2009}
\begin{enumerate}
...
\end{enumerate}
\item Match the joining processes in Group 1 with the filler materials in Group 2\hfill\brak{GATE-MT2009}
\begin{center}
\begin{tabular}{c c}
Group 1&Group 2\\
...
\end{enumerate}
\end{multicols}
\item  Match the properties in Group 1 with the metals in Group 2.\hfill\brak{GATE-MT2009}
\begin{center}
\begin{tabular}{c c}
Group 1 &Group 2\\
...
'impure copper' to pure copper using an electrolyte having $Cu^2+$ions at $298 K$ is\hfill\brak{GATE-MT2009}
\begin{multicols}{4}
\begin{enumerate}
\item $0.9 mV$
\item $9 mV$
...
\item As per the TTT diagram, bainite will form in eutectoid plain carbon steel when heated to $850^\circ C$ followed by\hfill\brak{GATE-MT2009}
...
\item  During low temperature plastic deformation of an under-aged precipitation hardened alloy, dislocations \hfill\brak{GATE-MT2009}
\begin{enumerate}
...
the two factors in the following list:\hfill\brak{GATE-MT2009}\\
...
\item At constant temperature and pressure, two phases $\alpha$ and $\beta$ will be in equilibrium when\hfill\brak{GATE-MT2009}\\
\begin{enumerate}
...
mould will be \hfill\brak{GATE-MT2009}
\begin{multicols}{4}
\begin{enumerate}
\item $0.147 mm$
\item $1.47 mm$
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.5\linewidth]{figs/Q.53.png}
    \label{fig:placeholder}
\end{figure}
