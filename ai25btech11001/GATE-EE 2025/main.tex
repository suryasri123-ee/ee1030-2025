...
\item {The above reaction is an example of}
		\begin{itemize}
			\item[(A)] addition reaction
			\item[(B)] bimolecular elimination reaction (E$_2$)
			\item[(C)] unimolecular substitution reaction (SN$_1$)
			\item[(D)] bimolecular substitution reaction (SN$_2$)

		\end{itemize}
	\item {If the concentration of KOH in the reaction mixture is doubled, the rate of the reaction will be}
		\begin{itemize}
			\item[(A)] decreased to one-half
			\item[(B)] the same 
			\item[(C)] increased by two-times
			\item[(D)] increased by four-times
