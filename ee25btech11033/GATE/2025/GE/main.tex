...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.17\textwidth]{figs/fig3.png}
...
\[
\Sigma =
% This outer array arranges the main components.
% The first column is for the row labels, right-aligned.
% The @{\hspace{0.5em}} adds a small space between the row labels and the matrix.
% The second column is for the matrix itself, left-aligned.
\begin{array}{r @{\hspace{0.5em}} l}
    & 
    % These are the column labels, placed above the matrix.
% We add horizontal spacing (\quad) to align them with the matrix columns.
\begin{array}{ccc}
        \text{band-$1$} & \quad \text{band-$2$} & \quad \text{band-$3$}
    \end{array} \\
    % This inner array holds the row labels vertically.
\begin{array}{r}
        \text{band-$1$} \\ \text{band-$2$} \\ \text{band-$3$}
    \end{array}
    & 
    % The bmatrix environment creates the matrix with square brackets.
\begin{bmatrix}
        34.14 & 46.71 & 40.68 \\
        46.71 & 68.83 & 69.59 \\
        40.68 & 69.59 & 248.40
